    ...
    <aside id="default-sidebar" class="fixed top-0 left-0 z-40 w-64 h-screen transition-transform -translate-x-full sm:translate-x-0" aria-label="Sidenav">
        <div class="flex flex-col justify-between h-full bg-white border-r border-gray-200 dark:bg-gray-800 dark:border-gray-700">
            ...
            <div class="overflow-y-auto px-4 py-5 space-y-4">
                <!-- Brand / Logo -->
                <a href="{{ auth()->check() && auth()->user()->isStaff() ? route('stock.tasks') : route('dashboard') }}" class="flex items-center space-x-3 px-2 pb-4 border-b border-gray-200 dark:border-gray-700 hover:opacity-90 transition">
                    @if ($logo)
